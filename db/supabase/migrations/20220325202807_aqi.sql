...
$$
begin
  insert into hourly_reading_stats
...
select hour,
       ...
       device_id,
       loc_id
from hourly_reading_stats
where chan_id = get_sensor_chan_id('O3');
...
select hour,
       ...
       loc_id
from hourly_reading_stats
where chan_id = get_sensor_chan_id('PM2_5');
...
select hour,
       ...
       loc_id
from hourly_reading_stats
where chan_id = get_sensor_chan_id('PM10');
...
declare
  ...
  aqi        int;
begin
  select trunc(conc::numeric, pm.trunc_amt)
  from pollutant pm
  where pm.name = pollutant
  into
    trunc_conc;
...
(
  ...
  timestamp     timestamp generated always as ( day + interval '1hr' * hour) stored,
...
begin
  if not new.chan_id in (
    select sensor_chan_id
    from sensor_chan sc
    where sc.name in ('O3', 'PM2_5', 'PM10', 'NO2', 'CO', 'SO2')) then
    return new;
  end if;

  select name
  from pollutant p
  where p.name = (select sc.name from sensor_chan sc where sc.sensor_chan_id = new.chan_id)
  into pol_name;

  select pollutant_id
  from pollutant p
  where p.name = pol_name
  into pol_id;

  select conc_to_aqi(pol_name, new.avg) into new_aqi;

  insert into raw_hourly_aqi (device_id, loc_id, pollutant_id, hour, day, aqi, type)
  values (new.device_id,
          new.loc_id,
          pol_id,
          new.hour,
...
from raw_hourly_aqi
where case
        when timestamp = date_trunc('hour', now()) then
          type = 'NOWCAST'
        else
          type = 'RAW' -- TODO: It should somehow prefer MIDPOINT over RAW, if MIDPOINT exists
        end
...
delete
from pollutant;
insert into pollutant (name, trunc_amt)
values ('O3', 3),
       ('PM2_5', 1),
       ('PM10', 0),
       ('CO', 1),
...
delete
from pollutant_subindex;
insert into pollutant_subindex (pol, timeframe_hours, aqi_low, aqi_high, conc_low, conc_high, advisory_statement)
values
  -- O3 8 hour
  ('O3', 8, 0, 50, 0, 0.054,
   'None.'),
