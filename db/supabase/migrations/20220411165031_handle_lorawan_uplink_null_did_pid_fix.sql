create or replace function handle_lorawan_uplink(req jsonb) returns json
  security definer
  language plpgsql
as
$$
...
begin
  ...
  perform event.emit('device_uplink'::text, pid::uuid,
                     jsonb_build_object(
                         'device_id', did,
                         'payload', payload,
                         'request', req
  ...
  exception
    when others then
      raise exception 'Error decoding cayenne payload %', payload;
  end;
...
revoke execute on function handle_lorawan_uplink(jsonb) from anon restrict;
revoke execute on function handle_lorawan_uplink(jsonb) from authenticated restrict;
