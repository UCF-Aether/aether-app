...
import { OriginBase, Distribution, BehaviorOptions, IOrigin, ErrorResponse } from 'aws-cdk-lib/aws-cloudfront';
...
export class Route53Domain extends StaticSiteDomain {
  ...
  getDistribution(scope: Construct, props: SiteDomainDistributionProps): Distribution {
    ...
    const zone = route53.HostedZone.fromLookup(scope, 'Zone', { domainName: domain });
    //s3-website-us-east-1.amazonaws.c
    // TLS certificate
    const dnsCertificate = Certificate.fromCertificateArn(scope, 'Certificate', this.dnsCertificateArn);
    new CfnOutput(scope, 'CertificateARN', { value: dnsCertificate.certificateArn });
...
export class StaticSite extends Construct {
  ...
  constructor(scope: Stack, id: string, props: StaticSiteProps) {
    ...
    const siteBucketName = props.domain instanceof Route53Domain ? props.domain.siteUrl() : `${id.toLowerCase()}-cloudfront-net`;
    ...
    const siteBucket = new s3.Bucket(this, 'SiteBucket', {
      ...
      // Use cloudfront redirects...if s3 is configured as a website, it expects the
      // *.s3-website-<region>.amazonaws.com URL to be available. But, that URL is only available if
      // the bucket is publically readable. 2-3 fucking hours wasted on this bullshit!
